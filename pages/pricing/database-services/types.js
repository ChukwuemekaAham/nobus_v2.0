...
function index() {
  ...
  const categories = ["overview","features","price","types"]

  return (
    <section>
      <Head>
        <title> Pricing - Nobus Database Service  </title>
          ...
          <Tab.Group defaultIndex={3}>
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4"> 
              ...
              <h2 className="pb-5 text-2xl font-semibold text-[#232f3e]">PRICING</h2>
                {categories.map((category, index) => (
                  ...
                  <Tab
                    key={category}
                    as={Link}
                    href={`/pricing/database-services/${category}`}
                    className={({ selected }) =>
                      classNames(
              ...
              <Tab.Panel data-headlessui-state="selected">
               <section id="types" className="mx-auto text-justify">
                    <p>
                      The operating systems currently available to use with your Nobus FCS instances include
